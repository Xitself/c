...
-- Create the TextLabel for the player distance
...
distanceLabel.TextYAlignment = Enum.TextYAlignment.Top-- Create the TextLabel for the player name
local nameLabel = Instance.new("TextLabel")
nameLabel.Parent = BillboardGui
nameLabel.BackgroundTransparency = 1
nameLabel.Position = UDim2.new(0, 0, 0, -50)
...
local Toggle = Tabs.Server:AddToggle("AutoRejoin", {
	...
	Callback = function(value)
        if value then 
            nexus:Notify({Title = 'Auto Rejoin', Content = 'You will rejoin if you are kicked or disconnected from the game', Duration = 1 })
            repeat task.wait() 
